...
Merge feature branches into `dev` with a merge commit after rebasing on `dev`.
...
This keeps each feature branch visible in the Git graph and makes rollback easier by reverting the merge commit.

Avoid fast-forward merges for `feat` into `dev` because they flatten feature boundaries into the integration branch.
